 ...
 *
 * Vollkorn — the body face — cannot be loaded by `@import` from
 * globals.css the way Marcellus and VT323 are: requesting its italic style
 * makes the Google Fonts URL carry a comma-separated axis list
 * (`ital,wght@0,400..900;1,400..900`), and the CSS pipeline in `next build`
 * drops an @import whose URL contains a comma, without a word in the build
 * log. The page then renders in the Georgia fallback, which reads as "the
 * type looks a bit off" rather than as a broken build. A <link> never goes
 * near that pipeline.
 *
